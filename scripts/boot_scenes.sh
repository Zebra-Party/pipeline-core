...
while IFS= read -r scene; do
	...
	# downstream HarfBuzz emits "Parameter "hb_font" is null" while text
	# is still laid out fine. These print from C / Engine.print_error_messages
	# can't mute them, so strip before grepping for real failures.
	filtered=$(grep -ivE 'FreeType: Error loading font|_ensure_cache_for_size|Parameter "hb_font" is null' "$output_file" || true)
